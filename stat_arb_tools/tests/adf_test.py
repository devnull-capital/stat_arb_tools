...
class TestADF(unittest.TestCase):
    def test_female_births(self):
        ...
        stat, pval = adf(delete(data.values, [0], axis=1).ravel())

        self.assertEqual(round(-4.808291253559763, 4), round(stat, 4))
        self.assertEqual(round(5.243412990149865e-05, 4), round(pval, 4))
...
class TestIsStationary(unittest.TestCase):
    def test_female_births(self):
        ...
        stat, pval = isStationary(appleList, googList)

        self.assertEqual(round(-1.3403293516072663, 4), round(stat, 4))
        self.assertEqual(round(0.6105256778014359, 4), round(pval, 4))
